...
void attack3(int m){//퍽이 3개 있을 경우 사용하는 함수 
	...
	if(m) V1(0,0,1,0,0,180,0,240,130,130,1);//매개변수가 들어오면 추적 
...
void M1()//시작 & 3개 퍽 처리하는 코드
{
	...
	//3개 퍽 처리 후 
	//출발/도착 벽 근처로 이동  
	LM(4,400,100,11,-100,150,-92,0);//현재 위치 : 맵지 기준(숫자 1) 
	FCC(2,0,1,165);//MidSET(psd[1]가 165만큼 붙을 때까지 이동) 
	
	//PUCK 쪽 벽 근처로 이동 
	Pm(0,0,0,165,650,10,0,0,400,0,0);
	Pm(0,0,1,135,650,00,0,0,250,0,0);
	TD(0,500,01,500,0,92,0);//현재 위치 : 맵지 기준(숫자 2) 
	HM(0,300,11,50,0,0);
	...
	TD(0,500,01,500,0,92,0);//현재 위치 : 맵지 기준(숫자 3) 
	HM(0,300,11,50,0,0);
...
void M2()//A구역 가변벽 코드
{
	...
	Pm(4,2,2,150,600,11,0,0,500,0,0);//psd[4]로 거리 500가기
...
void M4()//P3, P2, P1 처리 
{
	...
	Pm(0,0,0,130,500,10,0,0,250,0,0);//P2 처리하러 앞으로 이동
