...
void Puck6(int rl){//왼쪽시작이 1, 오른쪽 시작이 -1
	...
	for(int i=0;i<3;i++){
		if(tmp[i]==PUCK){
			if(i) LM(3,0,100,11,px[3],py[3],20*i*-rl,0);
...
void LastArea(int rl){//왼쪽이 1 오른쪽이 -1
	...
	HM(-30 * rl,300,10,170,0,0);
	TD(-30 * rl,300,01,100,200 * rl,-30 * rl,0);
	FCC(2,2,0,0);
	ping(1);
	V1(1,0,0,0,0,140,0,240,0,0,0);
	Pm(0,2,2,150,350,10,0,0,200,0,0);
	for(int i=0;i<2;i++){
		if(C_D[i][0] == PUCK){
			ping(3);
			...
			LM(3,0,100,11,0,0,0,0);
		}
	}
	LM(1,300,0,11,0,0,0,0);
	HM(0,300,11,-50,-100 * rl,0);
	turn(rl,200,11,90,0,0);
	Pm(0,2,2,150,400,11,0,0,300,0,0);
...
void Back(){//왼쪽으로나올때 G2 = 1 오른쪽이 G2 = -1
	...
	B[0] = 2;
	B[1] = 2;
	if(rl == 1 ? B[0] : B[1] == 2){//가장자리 2
		FCC(2,2,0,0);
		TD(0,400,10,350,-30 * rl,-90 * rl,0);
		TD(0,400,00,450,0,70 * rl,50);
		Pm(rl == 1 ? 4 : 5,2,2,150,400,00,1,rl == 1 ? 4 : 5,150,0,100);
		turn(1 * rl,100,11,20,0,0);
		rl == 1 ? FCC(1,2,3,230) : FCC(1,2,6,230);
	}
	Pm(0,0,rl == 1 ? 0 : 1,150,350,10,0,0,rl == 1 ? (Wall - 1) : (3 - Wall) * 300,0,0);
...
{	
	...
	Wall = 3;
	rl == 1 ? FCC(1,2,3,230) : FCC(1,2,6,230);
	Pm(0,2,2,150,350,10,0,0,rl == 1 ? (Wall - 1) : (3 - Wall) * 300,0,0);
	TD(0,400,11,450,-20 * rl,-93 * rl,0); 
	FCC(2,2,0,0);
}
void M5()
{
